...
func test_load_enemy_resources() -> void:
	...
	for id in enemies:
		...
		if enemy:
			...
			assert_gt(enemy.deck.size(), 0, "%s 有卡组" % id)
...
func test_map_generator_produces_valid_path() -> void:
	...
	assert_eq(nodes.size(), 11, "11 个节点（10 + boss）")

	# 节点 1-2 必须是 BATTLE
	assert_eq(nodes[0]["node_type"], int(MapGenerator.NodeType.BATTLE), "节点 1 是 BATTLE")
	assert_eq(nodes[1]["node_type"], int(MapGenerator.NodeType.BATTLE), "节点 2 是 BATTLE")

	# 节点 10 是 CAMPFIRE
	assert_eq(nodes[9]["node_type"], int(MapGenerator.NodeType.CAMPFIRE), "节点 10 是 CAMPFIRE")

	# 最后是 BOSS
	assert_eq(nodes[10]["node_type"], int(MapGenerator.NodeType.BOSS), "节点 11 是 BOSS")
...
func test_full_battle_using_resources() -> void:
	...
	push_warning("[e2e] 起手卡组 vs slime: %d ticks, %d damage, hp_left=%d" % [
...
func test_save_load_cycle() -> void:
	...
	if loaded:
		...
		assert_eq(loaded.map_nodes.size(), 11, "map_nodes 还原")

	# 清理
	save.delete_run()
	assert_false(save.has_active_run(), "delete_run 后 has_active_run 为 false")

# ─── 测试 5: 完整 11 节点通玩（阶段 1 验收） ──────────────────
...
func _try_run_seed(seed_val: int) -> Array[String]:
	...
	if nodes.size() != 11:
		...
		return log_lines
	if nodes[10]["node_type"] != int(MapGenerator.NodeType.BOSS):
		log_lines.append("INVALID last node type")
		return log_lines
	...
	for i in range(nodes.size()):
		...
		if nt == int(MapGenerator.NodeType.CAMPFIRE):
			...
			log_lines.append("node %02d  %-9s heal=%d hp=%d->%d" % [
				node_idx, nt_name, heal, hp_before, player.hp])
			continue

		var enemy := _spawn_enemy_for_node(node)
		if not enemy:
			log_lines.append("node %02d  ERROR: no enemy" % node_idx)
			break
		...
		if won:
			battles_won += 1
		log_lines.append("node %02d  %-9s enemy=%-12s ticks=%3d dmg=%3d taken=%3d hp=%d->%d won=%s" % [
			node_idx, nt_name, enemy.combatant_id, result.ticks_elapsed,
...
func test_save_load_midrun_preserves_state() -> void:
	...
	if loaded:
		...
		assert_eq(loaded.node_index, 6, "node_index 一致 (=6)")
		assert_eq(loaded.hp, 52, "hp 一致")
		assert_eq(loaded.max_hp, 80, "max_hp 一致")
		assert_eq(loaded.gold, 45, "gold 一致")
		assert_eq(loaded.map_nodes.size(), 11, "map_nodes 节点数 11")
		assert_eq(loaded.map_nodes[10]["node_type"], int(MapGenerator.NodeType.BOSS),
			"加载后 BOSS 仍在末位")
