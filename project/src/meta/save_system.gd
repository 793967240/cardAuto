...
func _rehydrate(state: RunState, data: Dictionary) -> void:
	...
	var relic_index := _build_relic_index()
	...
	for gid in gem_ids:
		if gid != "" and gem_index.has(gid):
			state.gems.append(GemInstance.new(gem_index[gid]))

	var raw_base_gems: Dictionary = data.get("base_gems", {})
	state.base_gems.clear()
	for s in state.bases:
		state.base_gems[s.id] = []
	var remaining_gems := state.gems.duplicate()
	for k in raw_base_gems:
		var bid := StringName(k)
		var gem_ids_arr: Array = raw_base_gems[k]
		var arr: Array = []
		for gid in gem_ids_arr:
			var gem_instance: GemInstance = _take_matching_gem_instance(remaining_gems, str(gid))
			if gem_instance != null:
				arr.append(gem_instance)
			elif gid != "" and gem_index.has(gid):
				arr.append(GemInstance.new(gem_index[gid]))
		state.base_gems[bid] = arr

	var relic_ids: Array = data.get("relics", [])
	state.relics.clear()
	for rid in relic_ids:
		if rid != "" and relic_index.has(rid):
			state.relics.append(relic_index[rid])
...
func _take_matching_gem_instance(gems: Array, gem_id: String):
	for i in range(gems.size()):
		...
		if gem is GemInstance:
			...
			if gi.data != null and str(gi.data.id) == gem_id:
				gems.remove_at(i)
				return gi
	return null
