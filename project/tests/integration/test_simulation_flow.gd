...
func test_short_cycle_bonus_heals_and_grants_gold_for_one_cycle_win() -> void:
	var run := _make_run_at_node(2, 50)
	run.gold = 25

	var bonus := BattleScene.apply_short_cycle_victory_bonus(run, 1)

	assert_eq(run.hp, 58, "1 周天内胜利应恢复 10% 最大体力")
	assert_eq(run.gold, 35, "1 周天内胜利应额外给 10 金币")
	assert_eq(bonus["healed"], 8, "返回实际恢复量")
	assert_eq(bonus["gold"], 10, "返回金币奖励")

func test_short_cycle_bonus_two_cycles_only_heals() -> void:
	var run := _make_run_at_node(2, 75)
	run.gold = 25

	var bonus := BattleScene.apply_short_cycle_victory_bonus(run, 2)

	assert_eq(run.hp, 80, "2 周天内胜利应恢复到体力上限")
	assert_eq(run.gold, 25, "2 周天胜利不应给 1 周天金币奖励")
	assert_eq(bonus["healed"], 5, "恢复量应按上限截断")
	assert_eq(bonus["gold"], 0, "金币奖励为 0")

func test_short_cycle_bonus_no_reward_after_three_cycles() -> void:
	var run := _make_run_at_node(2, 50)
	run.gold = 25

	var bonus := BattleScene.apply_short_cycle_victory_bonus(run, 3)

	assert_eq(run.hp, 50, "超过 2 周天不恢复体力")
	assert_eq(run.gold, 25, "超过 1 周天不加金币")
	assert_eq(bonus["healed"], 0)
...
func test_pick_simulation_enemy_targets_next_unfinished_battle_node() -> void:
	# 玩家停在节点 0（还没通关任何节点），可选起点中应能选到战斗敌人
	...
	var expected_id: String = available[0]["enemy_id"]
	assert_eq(enemy_id, expected_id, "应选到下一层可选战斗节点的 enemy_id")
...
func test_pick_simulation_enemy_skips_campfire() -> void:
	...
	run.map_nodes = [
		...
		},
		{
			...
			"floor": 2,
			"lane": 0,
			"node_type": int(MapGenerator.NodeType.BATTLE),
			"enemy_id": "slime",
			"next_ids": [],
		},
	]
	...
	var enemy_id := BuildScene.pick_simulation_enemy_id(run)
	assert_eq(enemy_id, "slime", "应跳过 CAMPFIRE 节点，选到下一个有 enemy_id 的节点")

func test_pick_simulation_enemy_falls_back_to_slime_when_no_battle_left() -> void:
	var run := _make_run_at_node(0)
	run.node_index = MapGenerator.FLOOR_COUNT
	run.current_node_id = "boss"
