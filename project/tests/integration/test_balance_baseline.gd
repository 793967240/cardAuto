...
# 用 BattleSimulator 跑批量战斗，验证起手卡牌仓库的胜率在合理区间
extends GutTest
...
func test_starter_vs_slime_baseline() -> void:
	...
	# 阶段 1 起手卡牌仓库 vs 弱敌应当稳赢
	assert_gte(win_rate, 0.9,
		"Starter card repository vs Slime: win rate should be >= 0.9 (got %.2f)" % win_rate)
...
func test_starter_vs_stone_guard_baseline() -> void:
	...
	# Act 1 普通战斗：起手卡牌仓库应当能打过石像卫（>= 40%）
	# 上限不严格断言：阶段 1 MVP 卡牌仓库偏强是可接受的，平衡留待阶段 2 调
	assert_gte(win_rate, 0.4,
		"Starter card repository vs Stone Guard: win rate should be >= 0.4 (got %.2f)" % win_rate)
