...
# ─── tick 推进测试 ────────────────────────────────────────────
# 注：GDScript lambda 不能修改外部 var（捕获是值），所以用 Array 单元素代替计数器

func test_tick_advances_at_1x_speed() -> void:
	var counter: Array[int] = [0]
	_timeline.tick_advanced.connect(func(_t): counter[0] += 1)

	# 1 秒 @ 1x = 2 tick（1 tick = 0.5s）
	_timeline.update(1.0)
	assert_eq(counter[0], 2, "1 second should advance 2 ticks at 1x speed")
...
func test_tick_advances_at_2x_speed() -> void:
	...
	_timeline.update(1.0)
	assert_eq(counter[0], 4, "1 second at 2x should advance 4 ticks")
...
func test_tick_advances_at_4x_speed() -> void:
	...
	_timeline.update(1.0)
	assert_eq(counter[0], 8, "1 second at 4x should advance 8 ticks")
...
func test_partial_second_accumulates() -> void:
	...
	assert_eq(counter[0], 0, "0.4 second should not advance any tick")

	# 再 0.2 秒，累计 0.6 秒 = 1 tick
	_timeline.update(0.2)
	assert_eq(counter[0], 1, "0.6 seconds total should advance 1 tick")
...
func test_advance_ticks_direct() -> void:
	...
	assert_eq(ticks_received, [1, 2, 3] as Array[int], "Tick values should be sequential")
