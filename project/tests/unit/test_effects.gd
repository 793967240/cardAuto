...
func test_strength_adds_damage_to_attack_cards() -> void:
	var p := _make_player()
	var e := _make_enemy(40)
	var ctx := _make_ctx(p, e)
	p.apply_status(StatusInstance.new(StatusInstance.ID_STRENGTH, 2, -1))
	var card := CardData.new()
	card.id = &"test_attack"
	card.cost = 1
	card.tags = [&"sword", &"attack"]
	var fx := EffectAttack.new()
	fx.damage = 5
	card.effect = fx
	...
	assert_eq(e.hp, 33, "Attack cards should gain +Strength damage")

func test_yang_jian_shi_grants_strength() -> void:
	var p := _make_player()
	var e := _make_enemy(40)
	var ctx := _make_ctx(p, e)
...
func test_ci_jian_gets_bonus_after_attack_card() -> void:
	...
	var e := _make_enemy(40)
	var ctx := _make_ctx(p, e)
	var starter := CardData.new()
	starter.id = &"starter_attack"
	starter.cost = 1
	starter.tags = [&"sword", &"attack"]
	var starter_fx := EffectAttack.new()
	starter_fx.damage = 1
	starter.effect = starter_fx
	var ci_jian := load("res://data/cards/sword/ci_jian.tres") as CardData
	p.chain.set_slots([CardRuntime.new(starter), CardRuntime.new(ci_jian)])

	p.chain.on_tick(ctx)
	p.chain.on_tick(ctx)

	assert_eq(e.hp, 32, "Starter attack deals 1, then ci_jian deals 4+3 after an Attack")

func test_ci_jian_without_previous_attack_deals_base_damage() -> void:
	var p := _make_player()
	var e := _make_enemy(40)
	var ctx := _make_ctx(p, e)
	var ci_jian := load("res://data/cards/sword/ci_jian.tres") as CardData
	p.chain.set_slots([CardRuntime.new(ci_jian)])

	p.chain.on_tick(ctx)

	assert_eq(e.hp, 36, "ci_jian should deal 4 when it has no previous Attack")
